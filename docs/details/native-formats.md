...
**Chat Completions**
- `max_tokens` 已废弃，推理模型用 `max_completion_tokens`（包含推理 token）[oa-chat]。
- `tool_calls[].function.arguments` 是字符串，不保证是合法 JSON，要自己校验 [oa-openapi]。
- 从 GPT-5.4 起，`reasoning_effort` 不是 `none` 时 Chat 不支持工具调用；部分新模型的工具调用只能走 Responses [oa-migrate]。
- OpenAI 的态度：Chat 继续支持，新项目推荐 Responses，没有下线日期 [oa-migrate][oa-deprec]。

**Responses**
- `previous_response_id` 串起来的历史照样按输入 token 计费 [oa-migrate]。
- `truncation` 默认 `"disabled"`，超出上下文直接 400；设 `"auto"` 会从最早的 items 开始丢 [oa-resp]。
- `background: true` 走异步，轮询 `GET /v1/responses/{id}`，可以用 `starting_after` 续上流 [oa-resp]。
- Assistants API 已于 2026-08-26 下线，替代方案是 Responses + Conversations [oa-deprec]。

**Anthropic Messages**
- `max_tokens: 0` 只写缓存、不生成 [an-msg]。
- 预填（最后一条放 assistant）会 400：工作指南写 Claude 4.6+，另一页写 Opus 4.7+（冲突）；4.7+ 采样参数只接受默认值，传别的会 400 [an-working]；Messages 参考页另写 `top_p` 可接受 ≥ 0.99、`top_k` 传任何值都拒绝，与指南不完全一致（冲突）[an-msg]。
- `tool_choice` 关并行用 `disable_parallel_tool_use`；哪些型号不接受 `any` / `tool` 见 [字段对照 · 工具](../field-atlas.md) [an-tools][an-errors][an-opus55]。
- 服务端工具不用你回传结果；跑到循环上限会返回 `pause_turn`，原样续发即可 [an-server-tools]。

**Gemini `generateContent`**
- 模型名写在 URL 路径里，不在请求体里 [g-gen]。
- 参考页用 camelCase（如 `finishReason`），官方 curl 示例里又有 snake_case（如 `system_instruction`）。Gemini 文档没说两种都收；Google API 底层的 proto3 JSON 规则要求服务端两种都接受，但没有针对 Gemini 实测 [g-gen][proto-json]。
- `Content.role` 写明只能 `user` / `model`，同页函数声明说明又提到 `"function"`（冲突，见 [冲突与未核实](conflicts.md)）[g-gen]。
- 被安全策略拦截时可能一个候选都没有，要看 `promptFeedback.blockReason` [g-gen]。
- Vertex AI：路径是 `/v1/projects/{P}/locations/{L}/publishers/google/models/{M}:generateContent`，用 OAuth Bearer；响应多出 `createTime` 和 `usageMetadata.trafficType` [vx-fc][vx-resp]。
...
| 历史 | 每次全量发 `contents` | 服务端默认记；`store=false` 时不能续，也不能后台执行（写法见 [导读](../llm-api-protocols.md)、[字段对照 · 工具](../field-atlas.md)，流式见 [字段对照 · 流式](../field-atlas.md)） |
...
来源：[g-int-ov][g-int-mig][g-int-blog][g-int-api][g-int-stream][g-int-err][g-err][g-cache2]。Gemini 的 OpenAI 兼容层（`/v1beta/openai/`）是另一个入口，文档没提它和 Interactions 的关系 [g-oai]。
